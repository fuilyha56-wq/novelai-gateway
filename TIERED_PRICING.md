...
网关会限制张数、步数、画面面积、参考图和 priority 等参数。该限制只能降低意外消耗风险，不能替代对 NovelAI 实际余额和上游规则变化的监控。V5 全部模型（含 `-limit`）还受网关侧每日 247 / 每周 1730 张限额约束（见 `NAI_V5_CRAWL_REPORT.md` 第 4.3 节）。
