...
配置共享凭据后，下游请求中的 `Authorization` 不再作为 NovelAI 上游凭据；网关统一使用配置的共享 NovelAI 凭据访问上游。该请求头现在用于验证调用方是否有权使用网关。
...
{"detail":"Gateway authentication is required when shared NovelAI credentials are configured. Set GATEWAY_PASSWORD, or explicitly set ALLOW_UNAUTHENTICATED_ACCESS=true only for a trusted private network."}
```

### 2.1 校验码的分发与轮换

当前实现使用一个固定的 `GATEWAY_PASSWORD` 作为网关校验码，不支持为不同调用方签发独立的下游 Key。因此：

- 直连 Gateway 的调用方必须获得同一个 `GATEWAY_PASSWORD`，并在每次 `/v1/*`、`/_api/*` 请求中发送 Bearer Header。
- 经 NewAPI 调用时，只把 `GATEWAY_PASSWORD` 配置到 **NewAPI → Gateway 渠道**；最终用户只持有自己的 NewAPI Key，不需要也不应拿到 `GATEWAY_PASSWORD`。
- 建议使用密码管理器生成至少 32 个随机字符，通过受控渠道分发。修改 `.env` 并重启 Gateway 即完成轮换，但旧校验码会同时对所有直连调用方失效。
- 不要把校验码写入公开前端、Git 仓库、日志、截图或请求 URL。URL 查询参数不受支持。

以下请求不要求 Bearer 校验码：CORS `OPTIONS` 预检、公开图片文件 `/images/{filename}`。管理端点和网页代理的规则见第 10 节。

### 2.2 NovelAI 多 Key 轮询规则
...
透明背景用法：透明由**提示词标签**驱动——在提示词里加 `transparent background, has alpha` 等标签（NAI V5 特性）。`transparent_background` / `tag_hint_transparent_background` 参数只是配合提示的 pass-through hint（官方 spec 原文 "the prompt is asking for a transparent background"），**单独传参数不会产生透明图**，网关也不会改动用户提示词。返回 PNG 保留 alpha 通道（网关仅对不透明图做白底压平，实质透明的图原样透传）。
...
| 放大 | `upscale` | `image`、`width`、`height`、`scale` | 当前统一入口返回 `400`；使用专用端点 |
| 控制图 | `annotate` | `image`、`model` | 当前统一入口返回 `400`；使用专用端点 |
...
带 `novelai_operation` 的统一图像入口始终返回 `application/json`。单图操作读取 `data[0].b64_json`；多图生成遍历完整 `data[]`。Director 工具的 PNG 不会重新编码：先前由 NovelAI 写入 PNG 的元数据会保留在 Base64 解码后的原始字节中。`usage` 仍按照实际操作的 Anlas 映射返回，Director 工具沿用固定的 5 或 65 Anlas。
...
| 设置项 | 值 | 原因 |
|---|---|---|
| Gateway 地址 | `http://<GATEWAY_HOST>:41555` | NewAPI 实际连接的上游地址 |
| 渠道 Key / Authorization | `GATEWAY_PASSWORD` 的值 | NewAPI 转发时必须生成 `Authorization: Bearer <GATEWAY_PASSWORD>` |
...
下游应用的 Base URL 指向 NewAPI 根地址，使用自己的 `<NEWAPI_API_KEY>` 请求标准路径 `/v1/images/generations`。NewAPI 渠道再用固定的 `GATEWAY_PASSWORD` 请求 Gateway；这两个 Key 用途不同，不能互换。每次调用在 JSON body 内传 `novelai_operation`，不能把它放进 `model`、`prompt` 或自定义 URL 路径。
...
下列请求都发送到同一个路径。替换 `<NEWAPI_HOST>`、`<NEWAPI_API_KEY>` 和图片占位符；此处不能填写 `GATEWAY_PASSWORD`。
...
`/v1/images/edits` 的 multipart 文件上传形式不能经这个统一 JSON 端点转发；需改为本文件给出的 JSON Base64 形式，再按上述规则加入 `"novelai_operation":"edits"`。单图结果从 `data[0].b64_json` 读取；生成请求使用 `n`/`n_samples>1` 时，按顺序读取整个 `data[]`。
...
| 值 | 响应 |
|---|---|
| `b64_json`、`auto` 或省略 | OpenAI JSON，每张图片对应一个 `data[i].b64_json` |
| `url` | OpenAI JSON，每张图片对应一个 `data[i].url`，指向网关 `/images/{filename}` |
...
当请求 `n` 或 `n_samples` 大于 1 时，网关会保留上游返回的全部 PNG，`data` 数组长度应与实际生成图片数一致，不再只返回第一张。`usage` 表示整次请求的总成本，而不是每个 `data[i]` 的单图成本。
...
获取 `response_format=url` 生成并落盘的 PNG。该路径用于让图片 URL 可直接显示，不要求 Bearer Header；因此不要把 `images/` 目录当作私密存储。路径穿越会返回 `403`，不存在返回 `404`。

### `ANY /_api/{path}`

NovelAI API 透明代理。支持 `GET`、`POST`、`PUT`、`DELETE`、`PATCH`、`OPTIONS`、`HEAD`。路径通过当前上游路由规则选择目标域名；重负载路径会记录生成统计。该入口与 `/v1/*` 使用相同的下游鉴权；CORS `OPTIONS` 预检除外。
...
| 401 | `/v1/*`、`/_api/*` 或管理端点缺少/使用了错误的 `GATEWAY_PASSWORD` |
| 403 | Chat/TTS 在配置中禁用，或上游权限不足 |
| 404 | 静态图片不存在，或上游不存在该路径 |
| 502 | 上游连接、上游二进制解包或代理失败 |
| 503 | 已配置共享 NovelAI 凭据但未配置 `GATEWAY_PASSWORD`，或服务不可用 |
...
`n_samples` 与 OpenAI 字段 `n` 使用同一套 1 至 6 校验，不能再通过只传 `n` 绕过数量限制。两者同时存在时以 `n_samples` 为准。请求多张图片时，默认 `b64_json`/`url` 响应会返回全部 `data[]`；调用方不能只处理 `data[0]`。
...
以下示例均以本机网关 `http://127.0.0.1:41555` 为例。将 `<GATEWAY_PASSWORD>` 替换为 `.env` 中固定的网关校验码，将 `<BASE64_IMAGE>`、`<BASE64_MASK>` 替换为真实 Base64 数据。
...
当生成响应中的 `data[i].url` 为 `/images/<filename>` 时，直接 GET 该 URL。此公开图片路径不要求 `GATEWAY_PASSWORD`：
...
原始 NovelAI API 路径前加 `/_api` 即可由网关转发。此模式不转换 OpenAI 请求或响应，也不返回网关 `usage` 映射；适用于已经实现 NovelAI 原生协议的调用方，但仍必须提供网关校验码。
...
2026-07-19 本地回归测试另外验证了本次请求层变更：

- `/v1/*` 与 `/_api/*` 在密码缺失/错误时返回 `401`，正确 Bearer 校验码可通过；CORS `OPTIONS` 预检不要求校验码。
- 配置共享 NovelAI 凭据但未设置 `GATEWAY_PASSWORD` 时返回 `503`；只有显式启用 `ALLOW_UNAUTHENTICATED_ACCESS` 才允许无鉴权访问。
- `n=7` 会在请求上游前返回 `400`，不能绕过 1 至 6 的样本数限制。
- 模拟的双图 ZIP 会完整返回两个 `data[]` 项；这项是响应转换回归测试，不代表再次进行了会扣 Anlas 的双图上游调用。
- `/ai/generate-image` 已验证路由到 `https://image.novelai.net`，不会再被宽泛的文本生成前缀错误匹配。
